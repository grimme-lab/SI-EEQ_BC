...
for method in args.methods:
    df[method + "_dev"] = abs(df[method] - df[REF_COL])
...
for method in args.methods:
    ...
    if method == "CEH-v1":
        ...
        method_specific_settings = {
            ...
            "marker": "*",
            "edgecolor": "black",
            # Label should be EEQ_{BC} (underscore BC)
            "label": r"EEQ$_{\text{BC}}$",
        }
    ...
    settings = {
        ...
        "label": method_specific_settings.get(
            "label", method
        ),  #  + f" ({pearsons[method]:.3f})",
    }
